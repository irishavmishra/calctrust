import type { BlogContent } from '@/lib/types/blog-content';

const content: BlogContent = {
  slug: '75000-salary-in-tennessee',
  category: 'state-salary',
  title: '$75,000 Salary in Tennessee',
  subheading: 'Calculate your take-home pay on a $75,000 salary in Tennessee. No state income tax.',
  metaTitle: '$75K Salary in Tennessee | Take-Home Pay 2025',
  metaDescription: 'Calculate take-home pay for a $75K salary in Tennessee. Estimates net income after federal and state taxes. Actual results may vary.',
  sections: {
    whatThisHelps: ['Understand take-home pay on $75K in TN', 'No state tax advantage'],
    whoUsefulFor: ['Tennessee workers', 'Nashville/Memphis employees'],
    howItWorks: ['$75K salary entered', 'No TN state tax', 'Only federal + FICA'],
    deepDive: ['$75,000 in Tennessee no state income tax = ~$4,000+ more per year vs. comparable tax states.'],
    examples: ['Annual: $75,000. Take-home: ~$4,900-5,100/month (no state tax).'],
    expertTips: ['TN no income tax is significant advantage', 'Nashville has booming economy'],
    assumptions: ['2025 federal rates', 'No state tax'],
    faq: [{ question: 'What is $75K take-home in TN?', answer: '~$4,900-5,100/month.' }],
    relatedCalculators: [{ title: '$100K Tennessee', href: '/usa/100000-salary-in-tennessee' }],
    freshnessSignal: 'December 2025',
  },
};

export default content;
